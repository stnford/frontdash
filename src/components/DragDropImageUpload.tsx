...
export function DragDropImageUpload({
  ...
  const handleDrop = (e: DragEvent<HTMLDivElement>) => {
    ...
    // ✅ Ensure .find callback param is typed as File
  ...
  const handleFileSelect = (e: ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0] as File | undefined; // ✅ narrow to File
    if (file) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        <Button type="button" variant="outline" onClick={onUseSample} className="flex-1">
          <ImageIcon className="w-4 h-4 mr-2" />
          Use Sample Image
        </Button>
        <Button type="button" variant="outline" onClick={openFileDialog} className="flex-1">
